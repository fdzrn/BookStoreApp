...
import com.bookstore.client.view.checkout.fragment.checkout.CheckoutFragment
import com.bookstore.client.view.checkout.fragment.payment.PaymentFragment
import com.bookstore.client.view.checkout.fragment.payment_success.PaymentSuccessFragment
...
class CheckoutActivity : AppCompatActivity() {
    ...
    private val paymentFragmentSuccess by lazy {
        ...
        checkoutViewModel.paymentResponse.observe(this, Observer { payment ->
            when(payment.status) {
                RetrofitStatus.SUCCESS -> supportFragmentManager.commit {
                    setCustomAnimations(R.anim.slide_in_right, R.anim.slide_out_left)
                    replace(fragment_container.id,paymentFragmentSuccess)
                }
                RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
