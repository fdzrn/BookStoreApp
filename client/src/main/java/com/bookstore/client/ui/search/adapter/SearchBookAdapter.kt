...
class SearchBookAdapter(private val searchBookItemListener: SearchBookItemListener) : RecyclerView.Adapter<SearchBookAdapter.ViewHolder>(), SearchBookFilterable {
    ...
    override fun performFilterByType(bookType: List<BookType>) {
        books = originBooks
        if (bookType.isNotEmpty()) books = originBooks.filter { originalBooks ->
            originalBooks.bookCategoryId == bookType.singleOrNull { bookType -> bookType.id == originalBooks.bookCategoryId }?.id
        }
        books.sortedBy { it.id }
        searchBookItemListener.onItemSearch(books.isEmpty())
        notifyDataSetChanged()
