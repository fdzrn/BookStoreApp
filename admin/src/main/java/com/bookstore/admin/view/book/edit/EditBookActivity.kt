                ...
                editBookViewModel.bookCategoryResponse.observe(this, Observer { response ->
                    ...
                    when(response.status) {
                        RetrofitStatus.SUCCESS -> response.list?.let { item ->
                            input_text_book_category.setText(book.bookCategory.name.replace("_", " ").capitalize().trim())
                            input_text_book_category.setAdapter(ArrayAdapter(this,R.layout.dropdown_menu_popup_item, item.map {it.name}))
                            setBookStatus(book)
                        }
...
//                            swipe_refresh_layout.isRefreshing = false
                        ...
                        RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                            ...
                            swipe_refresh_layout.isRefreshing = false
                            Snackbar.make(parent_layout, "There is an error when updating this book", Snackbar.LENGTH_SHORT).show()
                        }
                    }
                })
                ...
                editBookViewModel.uploadBookImageResponse.observe(this, Observer {
                    when(it.status) {
                        ...
                        RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                            ...
                            button_delete.isEnabled = true
                            swipe_refresh_layout.isRefreshing = false
                            Snackbar.make(parent_layout, "There is an error when uploading this book image", Snackbar.LENGTH_SHORT).show()
                        }
                    }
                })
                editBookViewModel.deleteBookResponse.observe(this, Observer {
                    when(it.status) {
                        RetrofitStatus.SUCCESS -> {
                            finish()
                            Toast.makeText(this, "Successfully delete book", Toast.LENGTH_SHORT).show()
                        }
                        RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                        else -> {
                            button_edit_book_cover.isEnabled = true
                            ...
                            button_delete.isEnabled = true
                            swipe_refresh_layout.isRefreshing = false
                            Snackbar.make(parent_layout, "There is an error when deleting this book", Snackbar.LENGTH_SHORT).show()
                        }
                    }
                })
                editBookViewModel.changeBookCover.observe(this, Observer {
                    Glide.with(this)
                        .load(it)
                ...
                swipe_refresh_layout.setOnRefreshListener { setContent(book) }
                button_back.setOnClickListener { onBackPressed() }
                button_edit_book_cover.setOnClickListener { performPickImage() }
                ...
                button_save.setOnClickListener {
                    ...
                    val category = editBookViewModel.currentBookCategory.firstOrNull {
                        it.name.toLowerCase() == input_text_book_category.text.toString().replace(" ", "_").toLowerCase()
                    }?.id
                    ...
                    input_book_category.error = if (category == null) "Please choose a book category" else null
                    input_book_author.error = if (author.isEmpty()) "Please input a book author name" else null
                    input_book_price.error = if (price.isEmpty()) "Please input a book price" else null
                    input_book_synopsis.error = if (synopsis.isEmpty()) "Please input a book synopsis" else null
    ...
    @SuppressLint("DefaultLocale")
    private fun setContent(book:BookModel) {
        swipe_refresh_layout.isRefreshing = true
        button_edit_book_cover.isEnabled = false
        button_save.isEnabled = false
        button_delete.isEnabled = false
    ...
    private fun setBookStatus(book: BookModel) {
        val bookStatus = editBookViewModel.getBookStatus()
        input_text_book_status.setText(book.bookStatus.replace("_", " ").toUpperCase(Locale.getDefault()))
        input_text_book_status.setAdapter(ArrayAdapter(this,R.layout.dropdown_menu_popup_item, bookStatus.map { it.toFormattedString() }))
        button_edit_book_cover.isEnabled = true
        button_save.isEnabled = true
        button_delete.isEnabled = true
