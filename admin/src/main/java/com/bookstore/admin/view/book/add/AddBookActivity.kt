package com.bookstore.admin.view.book.add
...
class AddBookActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
            ...
            when (response.status) {
                RetrofitStatus.SUCCESS -> response.list?.let { item ->
                    input_text_book_category.setAdapter(
                        ArrayAdapter(this, R.layout.dropdown_menu_popup_item, item.map { it.name })
                    )
                    setBookStatus()
                }
                RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                    ...
                    parent_layout,
                    "There is error when fetching book category",
        ...
        addBookViewModel.addBookResponse.observe(this, Observer { response ->
            when (response.status) {
                ...
                RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                    ...
                    Snackbar.make(
                        parent_layout,
                        "There is error when adding a book",
                        Snackbar.LENGTH_SHORT
                    ).show()
                }
            }
        ...
        addBookViewModel.uploadBookImageResponse.observe(this, Observer {
            ...
            when (it.status) {
                RetrofitStatus.SUCCESS -> {
                    finish()
                    Toast.makeText(this, "Successfully added a new book", Toast.LENGTH_SHORT).show()
                }
                RetrofitStatus.UNAUTHORIZED -> mainViewModel.logout(this)
                else -> Snackbar.make(
                    parent_layout,
        ...
        swipe_refresh_layout.setOnRefreshListener { refreshBookCategory() }
        ...
        button_save.setOnClickListener {
            ...
            input_book_category.error = if (category == null) "Please input a book author name" else null
            input_book_author.error = if (author.isEmpty()) "Please input a book synopsis" else null
    ...
    private fun refreshBookCategory() {
        ...
        button_edit_book_cover.isEnabled = false
        button_save.isEnabled = false
        input_text_book_category.isEnabled = false
    ...
    private fun setBookStatus() {
        ...
        input_text_book_status.setAdapter(
            ArrayAdapter(
                this,R.layout.dropdown_menu_popup_item,bookStatus.map{it.toFormattedString()}
            )
        )
        button_edit_book_cover.isEnabled = true
        button_save.isEnabled = true
