...
import com.bookstore.admin.model.formated.book.BookCategoryCountResponse
import com.bookstore.admin.model.formated.book.BookCountResponse
import com.bookstore.admin.model.formated.transaction.PurchaseCountResponse
...
class HomeViewModel(
    ...
    private val _bookResponseCount = MutableLiveData<BookCountResponse>()
    val bookCountResponse: LiveData<BookCountResponse> = _bookResponseCount

    fun getBookCount() = viewModelScope.launch(Dispatchers.IO) {
        try {
            val result = bookRepository.getBook()
            if (result.isNotEmpty()) _bookResponseCount.postValue(
                BookCountResponse(
                ...
                _bookResponseCount.postValue(BookCountResponse(RetrofitStatus.EMPTY))
                Log.e(this::class.java.simpleName, result.toString())
            }
        } catch (throwable: Throwable) {
            if (throwable is HttpException && throwable.code() == 401) {
                _bookResponseCount.postValue(BookCountResponse(RetrofitStatus.UNAUTHORIZED))
            } else {
                _bookResponseCount.postValue(BookCountResponse(RetrofitStatus.FAILURE))
